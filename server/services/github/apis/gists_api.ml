...
let gists_create ~inline_object_27_t () =
  ...
  let headers = Request.default_headers in
  let body =
    Request.write_as_json_body Inline_object_27.to_yojson inline_object_27_t
  in
...
let gists_create_comment ~gist_id ~inline_object_28_t () =
  ...
  let uri = Request.replace_path_param uri "gist_id" (fun x -> x) gist_id in
  let body =
    Request.write_as_json_body Inline_object_28.to_yojson inline_object_28_t
  in
  Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->
  Request.read_json_body_as
    (JsonSupport.unwrap Gist_comment.of_yojson)
    resp body
...
let gists_list ?since ?(per_page = 30l) ?(page = 1l) () =
  ...
  Request.read_json_body_as_list_of
    (JsonSupport.unwrap Base_gist.of_yojson)
    resp body
...
let gists_list_commits ~gist_id ?(per_page = 30l) ?(page = 1l) () =
  ...
  Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
  Request.read_json_body_as_list_of
    (JsonSupport.unwrap Gist_commit.of_yojson)
    resp body
...
let gists_list_forks ~gist_id ?(per_page = 30l) ?(page = 1l) () =
  ...
  Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
  Request.read_json_body_as_list_of
    (JsonSupport.unwrap Gist_full.of_yojson)
    resp body
...
let gists_update_comment ~gist_id ~comment_id ~inline_object_29_t () =
  ...
  in
  let body =
    Request.write_as_json_body Inline_object_29.to_yojson inline_object_29_t
  in
  Cohttp_lwt_unix.Client.call `PATCH uri ~headers ~body >>= fun (resp, body) ->
  Request.read_json_body_as
    (JsonSupport.unwrap Gist_comment.of_yojson)
